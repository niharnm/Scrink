...
final class VPNManager: ObservableObject {
    ...
    func appendLog(_ msg: String) {
        ...
        if statusLog.count > RinklerConstants.maxStatusLogEntries {
            statusLog.removeFirst(statusLog.count - RinklerConstants.maxStatusLogEntries)
        }
    }
        ...
        NETunnelProviderManager.loadAllFromPreferences { [weak self] managers, error in
            Task { @MainActor [weak self] in
                ...
                if let mgr = managers?.first(where: { candidate in
                    ...
                    return providerID == RinklerConstants.tunnelBundleID
                        || candidate.localizedDescription == RinklerConstants.vpnDescription
                }) {
                    self.manager = mgr
    ...
    private func createVPNProfile(startAfterSave: Bool) {
        ...
        proto.providerBundleIdentifier = RinklerConstants.tunnelBundleID
        proto.serverAddress = RinklerConstants.vpnServerAddress
        newManager.protocolConfiguration = proto
        newManager.localizedDescription = RinklerConstants.vpnDescription
