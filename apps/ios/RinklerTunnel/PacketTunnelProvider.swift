...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        let settings = NEPacketTunnelNetworkSettings(tunnelRemoteAddress: RinklerConstants.tunnelRemoteAddress)
        let ipv4 = NEIPv4Settings(
            addresses: [RinklerConstants.tunnelLocalAddress],
            subnetMasks: [RinklerConstants.tunnelSubnetMask]
        )
        ipv4.includedRoutes = [NEIPv4Route.default()]
        settings.ipv4Settings = ipv4
        let dns = NEDNSSettings(servers: RinklerConstants.dnsServers)
        settings.dnsSettings = dns
        settings.mtu = RinklerConstants.mtu
        ...
        setTunnelNetworkSettings(settings) { error in
            ...
            proxy.start { startError in
                ...
                let config = """
                tunnel:
                  mtu: \(RinklerConstants.mtu)
                socks5:
                  port: \(proxyPort)
                  address: \(RinklerConstants.socksBindAddress)
                misc:
                  task-stack-size: \(RinklerConstants.tun2socksTaskStackSize)
                  tcp-buffer-size: \(RinklerConstants.tun2socksTCPBufferSize)
                  connect-timeout: \(RinklerConstants.tun2socksConnectTimeout)
                  read-write-timeout: \(RinklerConstants.tun2socksReadWriteTimeout)
                  log-level: info
                """
